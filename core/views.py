...
from .serializers import PostSerializer,PostImageSerializer,PostCommentSerializer,CommentReplaySerializer,PostLikeSerializer
...
class ReplayViewSet (ModelViewSet):
    ...
    def get_queryset(self):
        return PostReplay.objects.filter(comment_id=self.kwargs['comment_pk']).filter(comment_id__post_id= self.kwargs['post_pk'])
